...
<div
	class="fixed inset-0 z-50 flex items-center justify-center bg-surface-950/80 backdrop-blur-sm p-4"
	role="dialog"
	aria-modal="true"
	tabindex="-1"
	onclick={handleClose}
...
>
	...
	<div
		class="bg-surface-950 border border-surface-300/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden"
		role="dialog"
	...
	>
		<!-- Header -->
		<div class="flex items-center justify-between px-6 py-4 border-b border-surface-300/10">
			<h2 class="text-xl font-bold text-surface-100">Upload</h2>
			<button
				onclick={handleClose}
		...
		<div class="p-6 overflow-y-auto" style="max-height: calc(90vh - 200px)">
			...
			<div
				bind:this={dropZone}
				class="relative border-2 border-dashed border-surface-200/20 rounded-xl p-8 text-center transition-all hover:border-surface-300/40"
				role="button"
				tabindex="0"
			...
			<!-- Category Input -->
			<div class="mt-6">
				<label for="category-input" class="block text-sm font-medium text-surface-300 mb-2">
					Category
				</label>
				<input
					id="category-input"
					...
					class="w-full px-4 py-3 bg-surface-500/5 border border-surface-300/10 rounded-lg text-surface-50 placeholder-surface-700 focus:outline-none focus:border-primary-500/50 transition-colors"
			...
			{#if galleryService.categories.length > 0}
				<div class="mt-3">
					<p class="text-xs text-surface-600 mb-2">Existing categories</p>
						...
						{#each galleryService.categories as cat}
							<button
								...
								class="px-3 py-1 text-xs rounded-full transition-colors {category === cat
									...
									: 'bg-surface-500/5 text-surface-50/60 border border-surface-300/10 hover:bg-surface-400/10 hover:text-surface-50/80'}"
							>
								{cat}
							</button>
						{/each}
			...
			{#if isUploading}
				<div class="mt-6">
					<div class="flex items-center justify-between text-sm mb-2">
						<span class="text-surface-300">Processing...</span>
						<span class="text-primary-400">{Math.round(uploadProgress)}%</span>
					</div>
					<div class="h-2 bg-surface-400/10 rounded-full overflow-hidden">
		...
		<!-- Footer -->
		<div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-surface-300/10">
			<button
				type="button"
				onclick={handleClose}
				class="px-4 py-2 text-surface-300 hover:text-surface-50/100 transition-colors"
				disabled={isUploading}
			>
				Cancel
			</button>
